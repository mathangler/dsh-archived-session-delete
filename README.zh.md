...
被替换按钮**相同的两个定宽槽位**，所以两次点击之间指针目标不会移动。行只会在宿主
确认之后才移除；若失败，它**原地不动**并回到初始按钮。

结果统一显示在**固定在面板底部的提示区**，一次只有一条：成功与扫描结果 3 秒后自动
消失并提供关闭按钮，失败则常驻，直到你关闭它或被下一条提示替换。提示区是 section 的
**最后一个子元素**，因此它是在所有行之后追加高度，而不是把某一行往下顶——
**报告结果永远不会移动任何一行**。该区域是 `aria-live`。

列表下方是**孤儿会话**，用分隔线区隔，并提供**扫描**按钮（扫描会遍历所有会话目录，
因此按需触发而非自动执行），其结果也在同一个提示区里报告。
...
- **删除的几次写入有固定顺序**，而且这个顺序是必需的：
  ...
  归档条目，都不可以。
- **任何东西都不会被渲染进列表里。** 每张列表都只是其数据源经搜索框过滤后的投影，
  因此行的位置只由该数据源的顺序决定。早前的实现会把被操作的行（及其结果）从客户端
  快照**按记住的索引插回**列表——而那个索引是按「列表里仍包含该已删除行」的状态测量
  的。于是下一次交互捕获到的是一个已不存在的位置，第二次删除的行就因此**错开一格**。
  去掉记住的索引、去掉快照插回、去掉滞留窗口，整类问题就消失了：**不存在第二个写入者
  会与列表顺序发生分歧**。`tools/orphan-path-check.mjs` 正是驱动你报告的这条序列，
  并且**在旧版本上会失败**。
...
- **投影缓存通过它所属的 domain 清除，而不是去删那个文件。** `session_projcache` 是
  **写回式（write-behind）**存储：记录活在 storage domain 的内存表里，`<id>.json` 只是
  它的持久化镜像。只删文件会留下内存记录，于是下一次写回——5 秒检查点、`turn/end`，甚至
  仅仅是第一次冷读——就把文件原样写回来，删除被**悄悄撤销**。现在改为调用
  `storageDomain.get('session_projcache').table('sessions').delete(id)`，一次同时清掉
  记录与镜像，之后才清扫残余文件。这与索引剪除遵循的是同一条规则：**走活动持有者，
  而不是动文件。** 缓存记录是缓存而非真相，因此 domain 缺失或降级不算错误——删除照常
  继续，只是改为清理文件。
...
- **孤儿检测是按需的**，不是后台扫描。
- **仍然「活着」的会话，其侧边栏条目要等宿主重启后才消失。** 这是插件**唯一修不了**的
  限制，而且它是平台边界，不是本插件的问题。UI 的会话列表是
  `sessionQuery.listSessions()`，它把**宿主内存中的会话 store 与磁盘结果合并**，所以
  只要该会话在本进程里被激活过（**在界面上打开过就算**），文件删掉后它依然会被列出。
  公开途径都去不掉它：store 没有 forget/detach/delete，唯一能摘掉条目的 disposer 是
  **创建它的 agent 生命周期持有的 capability**，插件拿不到。`workspace/session-stop` 只
  取消工作，`AgentHandle.dispose()` 仅限持有者，emit `session/disposed` 不会移除任何东西。
  DSH 自带的 `clean-dsh-sessions` 脚本最后也正是叫你去 **重启 `dsh web`**。

  所以本插件改为做它能做的：**上报**该会话是否仍存活（响应里的 `live` 字段），并在成功
  提示后追加一句说明，让这个残留入口变得有解释而不是莫名其妙。**删除本身是完整的**——
  数据、缓存记录与所有索引条目都已移除。
