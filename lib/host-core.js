...
export function createHandlers(ctx, options) {
   ...
   * @throws when the directory exists but cannot be read (permissions), so a
   *   silent empty answer can never be mistaken for "nothing to delete".
   */
  ...
  async function scanSessions() {
    const projects = [];
    for (const entry of await readEntries(sessionsRoot())) {
      if (entry.isDirectory()) projects.push(join(sessionsRoot(), entry.name));
    }

    const found = [];
    for (const project of projects) {
      for (const entry of await readEntries(project)) {
