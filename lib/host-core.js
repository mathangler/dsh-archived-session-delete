...
export function createHandlers(ctx, options) {
  ...
  return {
     ...
     * index entries.
     *
     * ORDER MATTERS for what the user sees. The grouped Session list shows a
     * session when it is owned by a Workspace AND not archived. Unarchiving
     * first would make the session briefly owned-and-unarchived, i.e. visible,
     * until the detach arrived a moment later — a temporary extra row users
     * see as a flicker. Detaching first removes ownership, so the session is
     * hidden by ownership before the archive flag changes and no frame ever
     * shows it. Both writes touch independent records and are idempotent, so
     * the final durable state is identical either way.
     *
     * @param args - `{ sessionId }`.
     */
    async delete(args) {
      ...
      try {
        ...
        if (reg !== undefined) {
          ...
          if (reg.archivedSessionIds.indexOf(id) !== -1) {
