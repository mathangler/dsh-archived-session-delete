...
export function createHandlers(ctx, options) {
  ...
  /**
   * Whether one session is still live in this host process.
   *
   * Reported to the caller, never acted on: the live Session store exposes no
   * forget/detach/delete method, and the one disposer that removes an entry is a
   * capability held by the agent lifecycle that created it, which a plugin
   * cannot obtain. `sessionQuery.listSessions()` merges the live store into its
   * result, so a session that was ever activated in this process keeps being
   * listed after its files are gone — which is why the UI needs a host restart
   * to forget it. Surfacing that here is the honest thing to do; the plugin
   * cannot fix it.
   *
   * @param id - the session to probe.
   * @returns whether the id is present in the live store.
   */
  function isLive(id) {
  ...
  /**
   * Evict one projection-cache record through its owning storage domain.
   *
   * The projection cache is WRITE-BEHIND: the record lives in the domain's
   * in-memory table and the `<id>.json` file is only its durable image. Deleting
   * the file alone — which is what this module used to do — left the record in
   * memory, so the next write-back (a 5 s checkpoint, a `turn/end`, or merely
   * the first cold read) recreated the file and the deletion silently came
   * undone. `KvTable.delete` drops both the record and the image, which is the
   * same "go through the live owner, not the file" rule the registry pruning
   * already follows.
   *
   * A cached record is a cache, not truth: failing to evict it is recoverable,
   * so every unavailable/degraded case returns `false` and the caller still
   * removes whatever files remain.
   *
   * @param id - the session whose record to evict.
   * @returns whether the domain reported removing a record.
   */
  async function evictProjectionRecord(id) {
    const facility = ctx.get('storageDomain');
    if (facility === undefined || typeof facility.get !== 'function') return false;
    let domain;
    try {
    ...
    if (domain === undefined || typeof domain.table !== 'function') return false;
    try {
      return (await domain.table('sessions').delete(id)) === true;
    } catch (error) {
      return false;
    }
  }
  ...
  return {
    ...
    async delete(args) {
      ...
      try {
        ...
        // Evict the projection record through its owning domain BEFORE removing
        // files. The cache is write-behind, so deleting the image first would
        // leave the in-memory record behind and the next checkpoint, turn end or
        // cold read would write the file straight back.
        if (await evictProjectionRecord(id)) steps.push('evicted projcache record via storageDomain');
        ...
        // Probed AFTER the writes so it describes the state the user is left
        // with: a still-live session keeps being listed by the UI because the
        // live store is merged into the session list, and only a host restart
        // clears it. Reported as data; the caller decides what to say.
        return { ok: true, id: id, live: isLive(id), steps: steps };
      } catch (error) {
