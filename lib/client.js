...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.asdel-notice{position:sticky;bottom:0;box-sizing:border-box;display:flex;align-items:center;gap:8px;margin-top:14px;padding:8px 10px;border:.5px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-layer-1);font-size:12px;line-height:18px}',
      '.asdel-notice-ok{color:var(--dsw-alias-state-success-primary)}',
      '.asdel-notice-err{color:var(--dsw-alias-state-error-primary)}',
      '.asdel-notice-icon{flex:none;width:14px;display:inline-flex;align-items:center;justify-content:center}',
      '.asdel-notice-text{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
      ...
      function commit(row) {
        ...
        callHost('delete', { sessionId: row.id }).then(
          function (value) {
            ...
            showNotice('success', t('deleted', { title: row.title }));
          },
          function (reason) {
            markBusy(row.id, false);
